...
Além de identificar trabalho no mesmo dia, o sistema compara os horários oficiais: `M` (07h–13h), `N6` (19h–01h), `PD` (07h–19h), `PN` (19h–07h), `T4` (14h–18h), `T` (13h–19h) e `P` (07h–07h do dia seguinte). `A51` é licença maternidade, `F1` férias, `F114` atestado médico e `F128` falta de plantão de 12h. O prefixo `E*` indica plantão extra, por exemplo `E*PD` e `E*PN`; o sistema remove esse prefixo apenas para determinar a faixa horária, preservando o código original na tabela. Assim, `M` em um vínculo e `T` no outro são classificados como **mesmo dia, mas sem sobreposição**; `PD` e `M` são classificados como **sobreposição real**. Quando o código não informa uma faixa horária conhecida, o resultado fica como **horário não determinado** para conferência manual.
